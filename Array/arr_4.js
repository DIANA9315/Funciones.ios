...
// Función para eliminar un número específico del arreglo
function eliminarNumero(valor) {
  const num = Number(valor);
  const index = numeros.indexOf(num);
  if (index !== -1) {
    numeros.splice(index, 1);
    console.log(`Número ${num} eliminado del arreglo.`);
  } else {
    console.log(`Número ${num} no encontrado en el arreglo.`);
  }
}
...
function mostrarMenu() {
  ...
  while (continuar) {
    const opcion = prompt(
      ...
      "5. Eliminar un número específico\n" +
      "0. Salir\n" +
      "Elige una opción:"
    );
    ...
    switch (opcion) {
      ...
      case "4":
        ...
        break;
      case "5":
        const valorEliminar = prompt("Ingresa el número a eliminar:");
        eliminarNumero(valorEliminar);
        break;
      case "0":
        continuar = false;
